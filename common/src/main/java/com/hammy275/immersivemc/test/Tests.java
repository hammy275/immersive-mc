...
public class Tests {
    ...
    public void runTests() {
        for (Test test : TESTS) {
            List<Method> methods = Arrays.stream(test.getClass().getDeclaredMethods())
                    ...
                    .toList();
            player.sendMessage(new TextComponent(test.getClass().getSimpleName()).withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.UNDERLINE), player.getUUID());
        ...
        if (passed == 0 && failed == 0) {
            player.sendMessage(new TextComponent("[!] No tests ran!").withStyle(ChatFormatting.YELLOW).withStyle(ChatFormatting.BOLD), player.getUUID());
        } else if (failed == 0) {
            player.sendMessage(new TextComponent("[✓] All tests passed!").withStyle(ChatFormatting.GREEN).withStyle(ChatFormatting.BOLD), player.getUUID());
        } else {
            player.sendMessage(new TextComponent("[✘] %d tests passed and %d tests failed!".formatted(passed, failed)).withStyle(ChatFormatting.RED).withStyle(ChatFormatting.BOLD), player.getUUID());
        }
    }
    ...
    private void logTestResult(Method testMethod, TestResult result) {
        ...
        player.sendMessage(new TextComponent(result.makeOutputMessage(testMethod.getName())).withStyle(style), player.getUUID());
        if (result.passed()) {
            passed++;
        } else {
            failed++;
