...
public class ImmersivesCustomizeScreen extends Screen {
    ...
    protected void init() {
        ...
        ScreenUtils.addOption("disable_vanilla_guis", config -> config.disableVanillaInteractionsForSupportedImmersives, (config, newVal) -> config.disableVanillaInteractionsForSupportedImmersives = newVal, this.list);
        ...
        if (ConfigScreen.getAdjustingConfigType() == ConfigType.CLIENT) {
            this.list.addBig(ScreenUtils.createEnumOption(CrouchMode.class, "config.immersivemc.crouch_mode",
                    mode -> new TranslatableComponent("config.immersivemc.crouch_mode." + mode.ordinal()),
                    mode -> new TranslatableComponent("config.immersivemc.crouch_mode." + mode.ordinal() + ".desc"),
            ...
            this.list.addBig(ScreenUtils.createIntSlider(
                    ...
                    val -> new TranslatableComponent("config.immersivemc.text_scale_val", String.format("%.2f", val / 20f)),
                    10, 40,
