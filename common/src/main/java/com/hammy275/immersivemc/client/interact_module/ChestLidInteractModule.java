...
public class ChestLidInteractModule implements HeldInteractModule {
    ...
    public boolean onHoldTick(LocalPlayer player, InteractionHand hand) {
        ...
        Vec3 start = chestBottomCenter.add(0, 0.625, 0).add(Vec3.atLowerCornerOf(info.forward.getNormal()).scale(0.5));
        Vec3 end = chestBottomCenter.add(0, 1.125, 0).add(Vec3.atLowerCornerOf(info.forward.getOpposite().getNormal()).scale(0.5));
