...
public class EnchantingData {
    ...
    private MutableComponent translate(String key) {
        if (Platform.isModLoaded("zenith")) {
            return new TranslatableComponent(key.replaceFirst("apotheosis", "zenith"));
        }
    ...
    private MutableComponent makeStatComponent(float stat, float max, boolean hasItem, boolean isPercent) {
        ...
        if (!hasItem) {
            return new TextComponent("??.??%s/??.??%s".formatted(end, end)).withStyle(ChatFormatting.OBFUSCATED);
        }
        return new TextComponent("%.2f%s/%.2f%s".formatted(stat, end, max, end));
    }
    ...
    public static class ETableData {
        ...
        public void set(ETableStorage.SlotData data) {
            ...
            if (data.enchantmentHints().isEmpty()) {
                this.textPreviews.add(new TextComponent("???"));
            } else {
                for (int i = 0; i < data.enchantmentHints().size(); i++) {
                    ...
                    if (ench != null) {
                        ...
                        this.textPreviews.add(new TextComponent(ench.getFullname(data.enchantmentHintLevels().get(i)).getString() + end));
                    } else {
                        this.levelsNeeded = -1;
                        break;
                    }
