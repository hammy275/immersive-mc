...
/**
 * The type of relative positioning to use for an Immersive built with {@link BlockBasedImmersiveBuilder}. The API
 * guarantees the below modes won't be removed and won't change names in terms of backwards-compatibility. Anything
 * else, including the ordering of the modes and the amount of them are NOT part of the API, and may change at any time.
 */
public enum HitboxPositioningMode {
    /**
     * Positioning is done based on the direction the block faces, where the block does
     ...
     * +Z moves the hitbox towards the player. For example, ImmersiveMC uses this for furnaces.
     */
    HORIZONTAL_BLOCK_FACING, // Face that block is facing, excluding up and down face
    /**
     * Positioning is done from the top-center of the block, with the direction being
    ...
    /**
     * Positioning is done from the top-center of the block. All translations are done in literal
     * world movements (+X moves on the X-axis positively, etc.). ImmersiveMC uses this for the jukebox.
     */
    TOP_LITERAL, // Top of block face, literal orientation (x is always in-game x, etc.)
    /**
     * The same as TOP_PLAYER_FACING, but the direction is based on the block's facing
    ...
    /**
     * The same as HORIZONTAL_BLOCK_FACING, but positioning is done based on the
    ...
    /**
     * The same as HORIZONTAL_BLOCK_FACING, but if the block faces up or down, calculations
     * are done assuming the block faces south. This makes +X correspond to moving the hitbox west. ImmersiveMC
     * uses this for the barrel, mainly for positioning the handle.
     */
    ...
    /**
     * TOP_PLAYER_FACING if the player is above the block, and HORIZONTAL_PLAYER_FACING
     * otherwise. ImmersiveMC uses this for the hopper.
     */
    PLAYER_FACING_NO_DOWN, // Combination of HORIZONTAL_PLAYER_FACING and TOP_PLAYER_FACING
    /**
     * Positioning is done from the center of the block face opposite of the
     ...
     * ImmersiveMC uses this for the shulker box.
     */
    ...
    /**
     * Positioning is done the same as {@link #HORIZONTAL_BLOCK_FACING}, but the front is based on the attachment point
     * of the block, rather than the block's facing direction. For non-wall attachments, this instead is the opposite
     * of the attachment direction. ImmersiveMC uses this for the grindstone.
     */
