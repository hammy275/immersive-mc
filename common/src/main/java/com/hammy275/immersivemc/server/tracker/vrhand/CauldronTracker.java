...
public class CauldronTracker extends AbstractVRHandTracker {
    ...
    protected boolean shouldRunForHand(Player player, InteractionHand hand, ItemStack stackInHand, VRPose currentVRPose) {
        ...
        return player.level.getBlockState(new BlockPos(data.getPos())).getBlock() instanceof AbstractCauldronBlock ||
                (player.level.getBlockState(new BlockPos(data.getPos()).below()).getBlock() instanceof AbstractCauldronBlock
                && player.level.getBlockState(new BlockPos(data.getPos())).isAir());
    }
    ...
    protected void runForHand(Player player, InteractionHand hand, ItemStack stackInHand, VRPose currentVRData) {
        ...
        BlockState handState = player.level.getBlockState(new BlockPos(data.getPos()));

        BlockPos cauldronPos = handState.getBlock() instanceof AbstractCauldronBlock ? new BlockPos(data.getPos()) :
                new BlockPos(data.getPos()).below();
