...
public class CampfireTracker extends AbstractTracker {
    ...
    protected void tick(Player player) {
        ...
        for (InteractionHand hand : InteractionHand.values()) {
            ...
            if (recipe.isPresent() && info.get(hand.ordinal()) >= recipe.get().getCookingTime() / 2) { // Smelt the held controller's item if we reach cook time.
                toSmelt.shrink(1);
                boolean didGive = player.getInventory().add(recipe.get().getResultItem().copy());
                if (!didGive) {
                    Util.placeLeftovers(player, recipe.get().getResultItem().copy());
                }
    ...
    protected boolean shouldTick(Player player) {
        ...
        for (InteractionHand hand : InteractionHand.values()) {
            ...
            BlockPos pos = new BlockPos(controller.getPos());
