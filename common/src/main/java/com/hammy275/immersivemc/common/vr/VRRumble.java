    ...
    public static void rumbleIfVR(Player player, InteractionHand hand, float rumbleDuration) {
        ...
        if (VRVerify.playerInVR(player) && rumbleInVRConfigCheck(player)) {
            if (player instanceof ServerPlayer sp) {
                VR.ServerAPI.sendHapticPulse(sp, VRBodyPart.fromInteractionHand(hand), rumbleDuration);
            } else if (player.isLocalPlayer()) {
                VR.ClientAPI.triggerHapticPulse(VRBodyPart.fromInteractionHand(hand), rumbleDuration);
            }
        }
    }
