...
public abstract class ItemWorldStorageHandler<S extends NetworkStorage> implements WorldStorageHandler<S> {
    ...
    public boolean isDirtyForClientSync(ServerPlayer player, BlockPos pos) {
        if (WorldStoragesImpl.getS(pos, player.getLevel()) instanceof ItemStorage iws) {
    ...
    public void onStopTracking(ServerPlayer player, BlockPos pos) {
        if (WorldStoragesImpl.getWithoutVerificationS(pos, player.getLevel()) instanceof ItemStorage iws) {
            if (Util.isValidBlocks(this, pos, player.level)) {
                ...
                WorldStoragesImpl.removeS(pos, player.getLevel());
            }
        }
    }
