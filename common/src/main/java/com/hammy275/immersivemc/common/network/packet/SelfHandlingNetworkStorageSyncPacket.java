...
public record SelfHandlingNetworkStorageSyncPacket(SelfHandlingNetworkStorage storage) {
    ...
    public static void encode(SelfHandlingNetworkStorageSyncPacket message, FriendlyByteBuf buffer) {
        message.storage.encode(buffer);
    }

    public static SelfHandlingNetworkStorageSyncPacket decode(FriendlyByteBuf buffer) {
    ...
    public static void handle(final SelfHandlingNetworkStorageSyncPacket packet, ServerPlayer player) {
        if (player != null) {
            packet.storage.handleServer(player);
        } else {
