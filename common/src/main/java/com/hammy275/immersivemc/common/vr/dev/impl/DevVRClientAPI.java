...
public class DevVRClientAPI implements VRClientAPI {
    ...
    public void triggerHapticPulse(VRBodyPart vrBodyPart, float v, float v1, float v2, float v3) {
        ...
        player.displayClientMessage(new TextComponent("Client haptic pulse for %s and body part %s".formatted(player.getGameProfile().getName(), vrBodyPart)), false);
    }
    ...
    public boolean openKeyboard(OpenKeyboardContext openKeyboardContext) {
        Minecraft.getInstance().player.displayClientMessage(new TextComponent("Opened VR keyboard"), false);
        return true;
    }

    @Override
    public boolean closeKeyboard(CloseKeyboardContext closeKeyboardContext) {
        Minecraft.getInstance().player.displayClientMessage(new TextComponent("Closed VR keyboard"), false);
        return true;
    }
}
