...
public class DevVRServerAPI implements VRServerAPI {
    ...
    public void sendHapticPulse(ServerPlayer serverPlayer, VRBodyPart vrBodyPart, float v, float v1, float v2, float v3) {
        serverPlayer.sendMessage(new TextComponent("Server haptic pulse for %s and body part %s".formatted(serverPlayer.getGameProfile().getName(), vrBodyPart)), serverPlayer.getUUID());
    }
}
