...
public record StopTrackPacketWithOwner(Identifier handlerID, UUID owner) {

    public static void encode(StopTrackPacketWithOwner packet, RegistryFriendlyByteBuf buffer) {
        buffer.writeIdentifier(packet.handlerID).writeUUID(packet.owner);
    }

    public static StopTrackPacketWithOwner decode(RegistryFriendlyByteBuf buffer) {
        return new StopTrackPacketWithOwner(buffer.readIdentifier(), buffer.readUUID());
    }
